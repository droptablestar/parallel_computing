...
void printdata(int *data, int rank) {
    cout << "P" << rank << ":" << endl;
...
int main(int argc, char **argv) {
    ...
    int snd[SIZE], recv[SIZE];
    for (i=0; i<SIZE; i++) {snd[i] = rank; recv[i]=3;}
    ...
    ofstream filestr;
    if (rank ==0)
        filestr.open("res_allreduce.txt", fstream::app);
    ...
    for (i=0; i<MSGS; i++) {
        ...
        MPI::COMM_WORLD.Allreduce(snd, recv, SIZE, MPI_INT, MPI_MIN);
